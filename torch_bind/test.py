...
x = projections.proj_l11_col(y, eta)
assert abs(projections.norm_l11_col(x)-eta) <= 1e-4, "L11_col proj fail"
assert abs(torch.sum(x)-eta) <= 1e-4, "L11 proj fail"
...
x = projections.proj_l11_bilevel_col(y, eta)
assert abs(projections.norm_l11_col(x)-eta) <= 1e-4, "L11_col proj fail"
assert abs(torch.sum(x)-eta) <= 1e-4, "L11 proj fail"
...
x = projections.proj_l1infty_col(y, eta)
assert abs(projections.norm_l1infty_col(x)-eta) <= 1e-4, "L1infty proj fail"
assert abs(torch.sum(torch.max(x,0).values)-eta) <= 1e-4, "L1infty proj fail"
...
x = projections.proj_l1infty_bilevel_col(y, eta)
assert abs(projections.norm_l1infty_col(x)-eta) <= 1e-4, "L1infty proj fail"
assert abs(torch.sum(torch.max(x,0).values)-eta) <= 1e-4, "L1infty proj fail"
